...
KeyboardEvent::KeyboardEvent()
    ...
    , m_altGraphKey(false)
#if ENABLE(CEHTML)
    , m_wasInitializedWithKeyCode(false)
#endif
{
}
...
#if ENABLE(CEHTML)
void KeyboardEvent::initKeyEvent(const AtomicString& type, bool canBubble, bool cancelable, AbstractView* view,
                                      bool ctrlKey, bool altKey, bool shiftKey, bool metaKey, unsigned keyCode)
{
    if (dispatched())
        return;

    // Spec: eventType should be "keydown" or "keyup"
    if (type != eventNames().keydownEvent && type != eventNames().keyupEvent)
        return;

    initUIEvent(type, canBubble, cancelable, view, 0);

    m_keyLocation = keyCode;
    m_ctrlKey = ctrlKey;
    m_shiftKey = shiftKey;
    m_altKey = altKey;
    m_metaKey = metaKey;
    m_wasInitializedWithKeyCode = true;
}
#endif
...
{
#if ENABLE(CEHTML)
    if (m_wasInitializedWithKeyCode)
