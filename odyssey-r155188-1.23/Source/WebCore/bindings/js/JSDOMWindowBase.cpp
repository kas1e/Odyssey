...
#if OS(MORPHOS)
static VM* vm_static = 0;

void resetVM()
{
    delete vm_static;
    vm_static = 0;
    JSDOMWindowBase::commonVM();
}
...
VM* JSDOMWindowBase::commonVM()
{
    ...
    if (!vm_static) {
        ...
        vm_static = VM::createLeaked(LargeHeap).leakRef();
        vm_static->exclusiveThread = currentThread();
        initNormalWorldClientData(vm_static);
    }
