				...
				if((stream->AHImp = CreateMsgPort()))
				{
					if((stream->AHIio = (struct AHIRequest *) CreateIORequest(stream->AHImp, sizeof(struct AHIRequest))))
					{
						...
						if(!(stream->AHIDevice = OpenDevice(AHINAME, AHI_NO_UNIT, (struct IORequest *)stream->AHIio, NULL)))
						{
							...
							#ifdef __amigaos4__
							IAHI = (struct AHIIFace *)GetInterface(stream->localAHIBase, "main", 1L, NULL);
							#endif
...
MediaPlayer::SupportsType MediaPlayerPrivate::supportsType(const String& type, const String& codecs, const WebCore::KURL& url)
{
